...
if CLIENT then
    ...
    hook.Add("CreateMove", "Noclip.CreateMove.PBODY", CreateMove)
end
...
hook.Add("Move", "ipMove.PBODY", ipMove)
...
if SERVER then
    hook.Add("PreCleanupMap", "RemovePortalsIndividually.PBODY", function()
        for _, v in pairs(ents.FindByClass("prop_portal_pbody")) do
            v:CleanMeUp()
        end
    end)

    hook.Add("DoPlayerDeath", "RemovePortalsOnDeath.PBODY", function(victim)
        local bluePortal = victim:GetNWEntity("Portal:Blue_PBODY")
        local orangePortal = victim:GetNWEntity("Portal:Orange_PBODY")

        for _, v in ipairs(ents.FindByClass("prop_portal_pbody")) do
