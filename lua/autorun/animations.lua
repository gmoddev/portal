...
timer.Simple(0.1, function()
    ...
    function GAMEMODE:HandlePlayerDucking(ply, velocity)
        ...
        ply.CalcIdeal = velocity:Length2D() > 0.5 and ACT_MP_CROUCHWALK or ACT_MP_CROUCH_IDLE
        return true
    end
    ...
    function GAMEMODE:HandlePlayerVaulting(ply, velocity)
        if velocity:Length() < 1000 or ply:PortalGroundCheck() then return end
    ...
    function GAMEMODE:HandlePlayerDriving(ply)
        if ply:InVehicle() then
            ...
            if pVehicle.HandleAnimation ~= nil then
                if type(pVehicle.HandleAnimation) ~= "function" then return false end
