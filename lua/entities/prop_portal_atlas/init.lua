...
function ENT:CleanMeUp()
    -- Stop portal loop sounds
    self.portal_loop:Stop("portal_loop")
    self.portal_loop2:Stop("portal_loop2")

    -- Boot player from portal
    self:BootPlayer()

    -- Set up angles for particle effects
    local ang = self:GetAngles()
    ang:RotateAroundAxis(ang:Right(), -90)
    ang:RotateAroundAxis(ang:Forward(), 0)
    ang:RotateAroundAxis(ang:Up(), 90)

    -- Adjust position if on floor
    local pos = self:GetPos()
    if self:OnFloor() then
        pos = pos - Vector(0, 0, 20)
    end

    -- Determine particle effect based on portal type and color
    local function getParticleEffect(portalType, colorConVar)
        local colorValue = GetConVarNumber(colorConVar)
        if colorValue >= 14 then
            return "portal_gray_close"
        elseif colorValue >= 11 then
            return "portal_2_close_pbody"
        elseif colorValue >= 10 then
            return "portal_2_close_pink_green"
        elseif colorValue >= 8 then
            return "portal_2_close_atlas"
        elseif colorValue >= 7 then
            return "portal_1_close"
        elseif colorValue >= 6 then
            return "portal_1_close_atlas"
        elseif colorValue >= 2 then
            return "portal_1_close_pbody"
        elseif colorValue >= 1 then
            return "portal_2_close"
        else
            return "portal_2_close_pbody"
        end
    end
    ...
    if self.PortalType == TYPE_BLUE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_BLUE_ATLAS, "portal_color_ATLAS_1"), pos, ang, self)
    elseif self.PortalType == TYPE_ORANGE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_ORANGE_ATLAS, "portal_color_ATLAS_2"), pos, ang, self)
    end

    -- Emit closing sound
    local soundPrefix = snd_portal2:GetBool() and "weapons/portalgun/portal2/portal_close" or "weapons/portalgun/portal_close"
    self:EmitSound(soundPrefix .. math.random(1, 2) .. ".wav", 70)

    -- Remove entity
    self:Remove()
end

function ENT:MoveToNewPos(pos, newang)
    -- Boot players from the portals
    self:BootPlayer()
    local otherPortal = self:GetOther()
    if IsValid(otherPortal) then
        otherPortal:BootPlayer()
    end

    -- Set the angles and positions for particle effects
    local ang = self:GetAngles()
    ang:RotateAroundAxis(ang:Right(), -90)
    ang:RotateAroundAxis(ang:Forward(), 0)
    ang:RotateAroundAxis(ang:Up(), 90)
    ...
    -- Function to determine the particle effect based on portal type and color
    local function getParticleEffect(portalType, colorConVar)
        local colorValue = GetConVarNumber(colorConVar)
        if colorValue >= 14 then
            return "portal_gray_close"
    ...
    if self.PortalType == TYPE_BLUE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_BLUE_ATLAS, "portal_color_ATLAS_1"), effectpos, ang, nil)
    elseif self.PortalType == TYPE_ORANGE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_ORANGE_ATLAS, "portal_color_ATLAS_2"), effectpos, ang, nil)
    end
    ...
    if self:OnRoof() and (not self:IsHorizontal()) then
        newang.z = newang.z + (upsidedown:GetBool() and 180 or 0)
        self:SetAngles(newang)
    end

    -- Notify clients about the portal move
...
function ENT:SuccessEffect()
    -- Rotate angles for particle effects
    local ang = self:GetAngles()
    ang:RotateAroundAxis(ang:Right(), -90)
    ang:RotateAroundAxis(ang:Forward(), 0)
    ang:RotateAroundAxis(ang:Up(), 90)
    
    -- Adjust position if on the floor
    local pos = self:GetPos()
    if self:OnFloor() then
        pos = pos - Vector(0, 0, 20)
    end
    ...
    local function getParticleEffect(portalType, colorConVar)
        local colorValue = GetConVarNumber(colorConVar)
        if colorValue >= 14 then
            return "portal_gray_success"
        elseif colorValue >= 11 then
            return "portal_2_success_pbody"
        elseif colorValue >= 10 then
            return "portal_2_success_pink_green"
        elseif colorValue >= 8 then
            return "portal_2_success_atlas"
        elseif colorValue >= 7 then
            return "portal_1_success"
        elseif colorValue >= 6 then
            return "portal_1_success_atlas"
        elseif colorValue >= 2 then
            return "portal_1_success_pbody"
        elseif colorValue >= 1 then
            return "portal_2_success"
        else
            return "portal_2_success_pbody"
        end
    end

    -- Trigger particle effects based on portal type and color
    if self.PortalType == TYPE_BLUE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_BLUE_ATLAS, "portal_color_ATLAS_1"), pos, ang, self)
    elseif self.PortalType == TYPE_ORANGE_ATLAS then
        ParticleEffect(getParticleEffect(TYPE_ORANGE_ATLAS, "portal_color_ATLAS_2"), pos, ang, self)
    ...
    -- Determine and emit sound based on portal type and sound configuration
    local soundPrefix = snd_portal2:GetBool() and "weapons/portalgun/portal2/portal_open" or "weapons/portalgun/portal_open"
    local int = math.random(1, 2)

    if self.PortalType == TYPE_BLUE_ATLAS then
        self:EmitSound(soundPrefix .. "1.wav", 100)
    elseif self.PortalType == TYPE_ORANGE_ATLAS then
        if int == 1 then int = 3 end
        self:EmitSound(soundPrefix .. int .. ".wav", 100)
        if snd_portal2:GetBool() then
...
--Mahalis code // Low wuz here elegance is king
function ENT:CanPort(ent)
    ...
    if ent:IsPlayer() then
        return true
    elseif not ent.isClone and IsValid(phys) and not invalidClasses[c] and not classStartsWith(c, "func_") and not classStartsWith(c, "prop_door") then
        return true
    else
        return false
    end
...
function ENT:MakeClone(ent)
    if not self:GetNWBool("Potal:Linked", false) or not self:GetNWBool("Potal:Activated", false) then return end
...
--// Someone prob had to make this list by hand and im not going that -Low
function ENT:StartTouch(ent)
    ...
    local function isExcludedModel(model)
        for _, excludedModel in ipairs(excludedModels) do
    ...
    -- Early return for excluded models
    ...
    -- Early return for excluded classes
    for _, excludedClass in ipairs(excludedClasses) do
        if class == excludedClass then
            ent:SetPos(Vector(-500, -500, -500))
            return
        end
    end

    -- Check if the portal is linked and activated
    if not self:GetNWBool("Potal:Linked", false) or not self:GetNWBool("Potal:Activated", false) then
        return
    end

    -- Skip if entity is already in a portal
    ...
    -- Handle other entities entering the portal
