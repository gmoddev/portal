...
function ENT:SetEffects(type)
    ...
    if ballEnable:GetBool() and not useInstant:GetBool() then
        local color = type == TYPE_BLUE_PBODY and GetConVarNumber("portal_color_PBODY_1") or GetConVarNumber("portal_color_PBODY_2")
...
function ENT:AttachParticleEffect(type, effectType, color)
    local effectName = PARTICLE_EFFECTS_PBODY[effectType][color] or "portal_gray_projectile_" .. effectType
